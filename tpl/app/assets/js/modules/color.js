...
export default class Color {
    constructor() {
        this.carousel = '.js-color-slider';
        this.selector = '.js-color-select';
        this.init();
        this.selectColor();
    }
    init() {
    ...
    slider() {
        ...
        _carousel.on( 'change.flickity', (e,i)=> {
            $('.s-color__item').eq(_lastSelected).addClass('last-selected').siblings().removeClass('last-selected');
            _selector.eq(i).parent().addClass('active').siblings().removeClass('active');
            _lastSelected = i;
        });
        _carousel.flickity({
            freeScroll: false,
    ...
    selectColor() {
        $(this.selector).on('click', (e)=> {
            ...
            $(this.carousel).flickity( 'select', _index);
            //return false;
        })
    }
