 ...
 * binary data using the `random_int($min, $max)` PHP function
 *
 * @deprecated The random_int($min, $max) function is not a reliable source of randomness.
...
class MtRandGenerator implements RandomGeneratorInterface
{
    ...
    public function generate($length)
    {
        ...
        for ($i = 1; $i <= $length; $i++) {
            $bytes = chr(random_int(0, 255)) . $bytes;
        }
