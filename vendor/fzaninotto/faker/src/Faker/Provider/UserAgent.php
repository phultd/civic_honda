...
class UserAgent extends Base
{
    ...
    public static function chrome()
    {
        ...
        $platforms = array(
            ...
            '(' . static::windowsPlatformToken() . ") AppleWebKit/$saf (KHTML, like Gecko) Chrome/" . random_int(36, 40) . '.0.' . random_int(800, 899) . ".0 Mobile Safari/$saf",
            '(' . static::macPlatformToken() . ") AppleWebKit/$saf (KHTML, like Gecko) Chrome/" . random_int(36, 40) . '.0.' . random_int(800, 899) . ".0 Mobile Safari/$saf"
        );
    ...
    {
        $ver = 'Gecko/' . date('Ymd', random_int(strtotime('2010-1-1'), time())) . ' Firefox/' . random_int(35, 37) . '.0';
    ...
    {
        $platforms = array(
            '(' . static::linuxPlatformToken() . '; ' . static::randomElement(static::$lang) . ') Presto/2.' . random_int(8, 12) . '.' . random_int(160, 355) . ' Version/' . random_int(10, 12) . '.00',
            '(' . static::windowsPlatformToken() . '; ' . static::randomElement(static::$lang) . ') Presto/2.' . random_int(8, 12) . '.' . random_int(160, 355) . ' Version/' . random_int(10, 12) . '.00'
        );
    ...
    {
        return 'Mozilla/5.0 (compatible; MSIE ' . random_int(5, 11) . '.0; ' . static::windowsPlatformToken() . '; Trident/' . random_int(3, 5) . '.' . random_int(0, 1) . ')';
    }
