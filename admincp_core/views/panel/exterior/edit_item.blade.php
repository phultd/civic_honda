...
@section('title'){{ __('Edit Ngoại thất Item') }}@endsection

@section('content')

<h4>Edit Ngoại thất Item</h4>
...
<form action="{{ route('admincp.exterior.update.item', ['category'=>Request::route('category'),'item'=>$item->id]) }}" method="post">
	@csrf
...
<form id="form_delete" class="d-none" action="{{ route('admincp.exterior.delete.item', ['category'=>Request::route('category'),'item'=>$item->id]) }}" method="post">
	@csrf
	@method('delete')
</form>
