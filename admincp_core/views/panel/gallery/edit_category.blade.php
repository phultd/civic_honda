...
@section('title'){{ __('Edit Thư viện Category') }}@endsection

@section('content')

<h4>Edit Thư viện Category</h4><br>

<form action="{{ route('admincp.gallery.update.category',['category'=>$category->id]) }}" method="post">
	@csrf
...
<form id="form_delete" class="d-none" action="{{ route('admincp.gallery.delete.category', ['category'=>$category->id]) }}" method="post">
	@csrf
	@method('delete')
</form>
...
<h4 class="mt-5">Danh sách Thư viện Item</h4>

<div class="button-group mb-2">
	<a href="{{ route('admincp.gallery.add.item',['category'=>$category->id]) }}" type="submit" class="btn btn-sm btn-primary">Add New Item</a>
</div>
    ...
    <tbody>
		...
		@forelse( $items as $item )
			<tr class="text-center">
				<td>
					{{ $loop->iteration }}<br>
					<a class="btn-link text-primary" href="{{ route('admincp.gallery.edit.item', ['category'=>$category->id,'item'=>$item->id]) }}">
						Edit
					</a>
				</td>
                <td>
		...
		@empty
			<tr><td colspan="3">{{ __('No data to display.') }}</td></tr>
        @endforelse
