...
@section('title'){{ __('Nội thất - Tổng quan') }}@endsection

@section('content')

<h4>Nội thất - Tổng quan</h4><br>

<form action="{{ route('admincp.interior.update') }}" method="post">
	@csrf
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				@elseif( !empty($interior) && $interior->section_background )
					{{ $interior->section_background }}
				@endif">
				...
				<div class="image-preview">
					...
					@elseif( !empty($interior) && $interior->section_background )
						<img class="img-thumbnail" src="{{ url( $interior->section_background ) }}">
						<a type="button" class="btn-floating btn-sm btn-warning btn-wysiwyg-remove"><i class="fa fa-trash-o"></i></a>
					@else
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				@elseif( !empty($interior) && $interior->section_background_mobile )
					{{ $interior->section_background_mobile }}
				@endif">
				...
				<div class="image-preview">
					@if( old('section_background_mobile') )
						...
						<a type="button" class="btn-floating btn-sm btn-warning btn-wysiwyg-remove"><i class="fa fa-trash-o"></i></a>
					@elseif( !empty($interior) && $interior->section_background_mobile )
						<img class="img-thumbnail" src="{{ url( $interior->section_background_mobile ) }}">
						<a type="button" class="btn-floating btn-sm btn-warning btn-wysiwyg-remove"><i class="fa fa-trash-o"></i></a>
					@else
					<span>No image selected</span>
					@endif
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				value="@if( old('section_title') ){{ old('section_title') }}@elseif( !empty($interior) && $interior->section_title ){{ $interior->section_title }}@endif">
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				value="@if( old('section_description') ){{ old('section_description') }}@elseif( !empty($interior) && $interior->section_description ){{ $interior->section_description }}@endif">
...
<h4 class="mt-5">Danh sách Nội thất Category</h4>

<div class="button-group mb-2">
	<a href="{{ route('admincp.interior.add.category') }}" type="submit" class="btn btn-sm btn-primary">Add New Category</a>
</div>
...
<table class="table table-list table-striped table-bordered table-sm table-hover">
        ...
        <tbody>
			...
			@forelse( $interior_categories as $category )
				<tr class="text-center">
					<td>{{ $loop->iteration }}</td>
	                <td><a class="btn-link text-primary" href="{{ route('admincp.interior.edit.category', ['category'=>$category->id]) }}">{{ $category->title }}</a></td>
