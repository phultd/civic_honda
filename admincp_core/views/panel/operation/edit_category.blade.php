...
@section('title'){{ __('Edit Vận hành Category') }}@endsection

@section('content')

<h4>Edit Vận hành Category</h4><br>

<form action="{{ route('admincp.operation.update.category',['category'=>$category->id]) }}" method="post">
	@csrf
...
<form id="form_delete" class="d-none" action="{{ route('admincp.operation.delete.category', ['category'=>$category->id]) }}" method="post">
	@csrf
	@method('delete')
</form>
...
<h4 class="mt-5">Danh sách Vận hành Item</h4>

<div class="button-group mb-2">
	<a href="{{ route('admincp.operation.add.item',['category'=>$category->id]) }}" type="submit" class="btn btn-sm btn-primary">Add New Item</a>
</div>
    ...
    <tbody>
		...
		@forelse( $items as $item )
			<tr class="text-center">
				<td>
					{{ $loop->iteration }}<br>
					<a class="btn-link text-primary" href="{{ route('admincp.operation.edit.item', ['category'=>$category->id,'item'=>$item->id]) }}">
						Edit
					</a>
				</td>
                <td>
