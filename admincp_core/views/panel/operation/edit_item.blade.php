...
@section('title'){{ __('Edit Vận hành Item') }}@endsection

@section('content')

<h4>Edit Vận hành Item</h4>

<a href="{{ route('admincp.operation.edit.category',['category'=>Request::route('category')]) }}" class="btn btn-sm btn-light mb-5"><i class="fa fa-chevron-left"></i> Back to category <b>@if( !empty($category->title) ){{ $category->title }}@endif</b></a>

<form action="{{ route('admincp.operation.update.item', ['category'=>Request::route('category'),'item'=>$item->id]) }}" method="post">
	@csrf
...
<form id="form_delete" class="d-none" action="{{ route('admincp.operation.delete.item', ['category'=>Request::route('category'),'item'=>$item->id]) }}" method="post">
	@csrf
	@method('delete')
</form>
