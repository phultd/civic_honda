...
@section('title'){{ __('Vận hành - Tổng quan') }}@endsection

@section('content')

<h4>Vận hành - Tổng quan</h4><br>

<form action="{{ route('admincp.operation.update') }}" method="post">
	@csrf
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				@elseif( !empty($operation) && $operation->section_background )
					{{ $operation->section_background }}
				@endif">
				...
				<div class="image-preview">
					...
					@elseif( !empty($operation) && $operation->section_background )
						<img class="img-thumbnail" src="{{ url( $operation->section_background ) }}">
						<a type="button" class="btn-floating btn-sm btn-warning btn-wysiwyg-remove"><i class="fa fa-trash-o"></i></a>
					@else
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				@elseif( !empty($operation) && $operation->section_background_mobile )
					{{ $operation->section_background_mobile }}
				@endif">
				...
				<div class="image-preview">
					@if( old('section_background_mobile') )
						...
						<a type="button" class="btn-floating btn-sm btn-warning btn-wysiwyg-remove"><i class="fa fa-trash-o"></i></a>
					@elseif( !empty($operation) && $operation->section_background_mobile )
						<img class="img-thumbnail" src="{{ url( $operation->section_background_mobile ) }}">
						<a type="button" class="btn-floating btn-sm btn-warning btn-wysiwyg-remove"><i class="fa fa-trash-o"></i></a>
					@else
					<span>No image selected</span>
					@endif
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				value="@if( old('section_title') ){{ old('section_title') }}@elseif( !empty($operation) && $operation->section_title ){{ $operation->section_title }}@endif">
	...
	<div class="card mb-3">
		...
		<div class="card-body">
			<div class="md-form">
				...
				value="@if( old('section_description') ){{ old('section_description') }}@elseif( !empty($operation) && $operation->section_description ){{ $operation->section_description }}@endif">
...
<h4 class="mt-5">Danh sách Vận hành Category</h4>

<div class="button-group mb-2">
	<a href="{{ route('admincp.operation.add.category') }}" type="submit" class="btn btn-sm btn-primary">Add New Category</a>
</div>
...
<table class="table table-list table-striped table-bordered table-sm table-hover">
        ...
        <tbody>
			...
			@forelse( $operation_categories as $category )
				<tr class="text-center">
					<td>{{ $loop->iteration }}</td>
	                <td><a class="btn-link text-primary" href="{{ route('admincp.operation.edit.category', ['category'=>$category->id]) }}">{{ $category->title }}</a></td>
