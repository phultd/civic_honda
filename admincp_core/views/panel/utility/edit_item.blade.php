...
@section('title'){{ __('Edit Tiện ích Item') }}@endsection

@section('content')

<h4>Edit Tiện ích Item</h4>

<a href="{{ route('admincp.utility.edit.category',['category'=>Request::route('category')]) }}" class="btn btn-sm btn-light mb-5"><i class="fa fa-chevron-left"></i> Back to category <b>@if( !empty($category->title) ){{ $category->title }}@endif</b></a>

<form action="{{ route('admincp.utility.update.item', ['category'=>Request::route('category'),'item'=>$item->id]) }}" method="post">
	@csrf
...
<form id="form_delete" class="d-none" action="{{ route('admincp.utility.delete.item', ['category'=>Request::route('category'),'item'=>$item->id]) }}" method="post">
	@csrf
	@method('delete')
</form>
